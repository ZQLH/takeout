...
public class OnlinePayActivity extends BaseActivty {
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        Intent intent=new Intent(OnlinePayActivity.this,MainActivity.class);
    ...
    public void success(Object o) {
        if (o instanceof HashMap) {
            ...
            tvOrderDetail.setText(addressBean.name+" "+addressBean.sex+" "+addressBean.phone+" "+addressBean.receiptAddress+"/r/n"+addressBean.detailAddress);
    ...
    private void addPayment(List<PaymentInfo> payments) {
        for (PaymentInfo item : payments) {
            ...
            int h = (int) (TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()) + 0.5);
            ...
            Picasso.with(this).load(item.url.replace("172.16.0.116","47.94.91.212")).into((ImageView) view.findViewById(R.id.iv_pay_logo));
    ...
    @OnClick({R.id.ib_back, R.id.bt_confirm_pay})
    ...
    public void pay(View v) {
        // 封装订单参数
        String orderInfo = getOrderInfo(ShoppingCartManager.getInstance().name, "通过黑马程序员订购的", "0.01");
        // 使用《用户私钥SHA256》(在创建应用时,设置密钥时，生成的)， 对订单做 RSA 签名
        //注意： 支付宝会使用《支付宝的公钥》(在设置密钥时，上传公钥给支付宝服务器时，生成的)进行验签
